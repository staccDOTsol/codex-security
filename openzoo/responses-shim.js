/**
 * Responses API <-> chat completions, as used to run Codex Security through a
 * gateway that serves only chat completions.
 *
 * Vendored from openzoo-shim (lib/responses.js). Wire it into any OpenAI-shaped
 * proxy: translate on POST /v1/responses, forward as /v1/chat/completions, then
 * translate the reply back. Payment, caching and metering stay on one path.
 *
 * The four failure modes this exists to fix are documented in ./README.md.
 */
...
export function responsesToChat(body, meta = {}) {
    ...
    messages = body.input.map((part) => {
      if (part && typeof part === 'object') {
        ...
        if (part.type === 'custom_tool_call_output') {
          return { role: 'tool', tool_call_id: part.call_id, content: String(part.output ?? '') };
        }
  ...
  const out = {
    model: body.model,
    messages,
    // Responses calls it max_output_tokens. A missing cap here would silently
    // become the chat default, which is far smaller than a Responses caller
    // expects.
